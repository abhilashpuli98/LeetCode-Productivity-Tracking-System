...
import SyncService from "./scripts/services/sync-services.js";
...
class LeetCodeStateManager {
  ...
  broadcastState() {
    chrome.runtime
      ...
      .catch(err => console.debug("Popup not open:", err));
  }
}
...
class GitHubService {
  ...
  async getAllLeetCodeProblems() {
    try {
      ...
      const { leetcode_tracker_token } =
      await chrome.storage.local.get("leetcode_tracker_token");
      ...
      return data
        .filter((problem) => /^\d+-/.test(problem.name))
        .map((problem) => ({
...
class LeetCodeTrackerController {
   ...
   */
  async startSync() {
    console.log("[LeetCodeTrackerController.startSync] Sync requested");
    
    try {
      console.log("[LeetCodeTrackerController.startSync] Calling syncService.startSync()");
      const result = await this.syncService.startSync();
      console.log("[LeetCodeTrackerController.startSync] Sync result:", result);
      ...
      console.error("[LeetCodeTrackerController.startSync] Error during sync:", error);
...
console.log("[BACKGROUND] Script loaded, initializing LeetCodeTrackerController...");
const controller = new LeetCodeTrackerController();
console.log("[BACKGROUND] LeetCodeTrackerController initialized successfully");

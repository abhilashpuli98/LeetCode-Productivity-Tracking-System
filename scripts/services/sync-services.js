...
import GithubService from "./gitservice.js";
...
export default class SyncService {
   ...
   */
  async startSync() {
    console.log("[SyncService.startSync] Starting synchronization process...");
    ...
    if (leetcode_tracker_sync_in_progress) {
      console.log("[SyncService.startSync] Sync already in progress, aborting");
      return {
        success: false,
    ...
    try {
      console.log("[SyncService.startSync] Fetching solved problems from LeetCode...");
      const solvedProblems = await this.leetcodeService.getSolvedProblems();
      console.log("[SyncService.startSync] Found " + solvedProblems.length + " solved problems");
      ...
      console.log("[SyncService.startSync] " + successMessage);
  ...
  async processGithubQueue() {
    if (this.githubProcessing || this.githubQueue.length === 0) return;
      ...
      const startNextProblem = async () => {
        ...
        promise
          ...
          .catch((error) => {
            ...
            console.error(`Error processing problem ${currentIndex}:`, error);
  ...
  async processProblem(problem, index) {
    ...
    try {
      ...
      for (const lang in submissionsByLanguage) {
        ...
        // Normalize language name to lowercase for languageMap lookup
        // submission.lang contains the language key from LeetCode API (e.g., "python", "python3", "cpp")
        const normalizedLang = submission.lang ? submission.lang.toLowerCase().trim() : null;
        console.log(`[SyncService.processProblem] Original lang: "${submission.lang}", normalized: "${normalizedLang}"`);
        problemObj.language = LanguageUtils.getLanguageInfo(normalizedLang);
        console.log(`[SyncService.processProblem] Language lookup result:`, problemObj.language);
        problemObj.code = submission.code;

        // Skip if language is not supported
        if (!problemObj.language) {
          console.warn(`[SyncService.processProblem] Unsupported language: ${submission.lang}, skipping`);
          continue;
        }

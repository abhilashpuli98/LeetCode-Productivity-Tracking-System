  ...
  import GithubService from "/scripts/services/gitservice.js";
  ...
  export default class LeetcodeTracker {
     ...
     */
async init() {
  console.log("[LT] init called", location.pathname);

  this.problem.loadProblemFromDOM();

  const submitButton = await DOMUtils.waitForElement(
    domElements.submitButton
  );

  console.log("[LT] submit button found", submitButton);
...
if (!submitButton) {
  console.warn("[LT] Submit button not found");
  return;
}
      ...
      console.log("[LT] Submit button attached");
    }
    ...
    async handleSubmission() {
      ...
      if (accepted && accepted.textContent === "Accepted") {
        ...
        try {
          ...
          console.log("[LT] Submission accepted");
        } catch (error) {
